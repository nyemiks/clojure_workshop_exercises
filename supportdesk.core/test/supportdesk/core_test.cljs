(ns supportdesk.core-test
    (:require
     [cljs.test :refer-macros [deftest is testing]]
     [supportdesk.core :refer [multiply]]
	  [cljs.test :refer-macros [are deftest is testing use-fixtures]]
      [clojure.test.check.generators :as gen]
      [clojure.test.check.properties :refer-macros [for-all]]
      [clojure.test.check.clojure-test :refer-macros [defspec]]
      [cuerdas.core :as str]
      [supportdesk.utils :refer [delete-item-from-list-by-title get-priorities-list
       get-sort-message get-sorted-priorities-list handle-sort-finish]]
	 )
	 )
	 
	 
	 (use-fixtures :each {:before (fn [] (do (def priorities-list [{:title "IE bugs" :priority 2}
                               {:title "404 page" :priority 1} {:title "Forgotten username" :priority 2}
                               {:title "Login token" :priority 1} {:title "Mobile version" :priority 3} 
...
(is (= [{:title "404 page", :priority 1} {:title "Login token",
:priority 1}]
...
(is (= {:sort-counter 1}
((handle-sort-finish app-state) "event"
"object"))))
